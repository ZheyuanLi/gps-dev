...
gps2bspl <- function (x, xt, d, m, gps = TRUE, periodic = FALSE,
  ...
  Dt <- Csparse2LTB(D)
  Dt <- VecScal(sqrt(VecDot(B@x) / VecDot(D@x)), Dt, overwrite = TRUE)
  S <- DtD(Dt)
  if (ncol(S) > p) S <- S[, 1:p]
  if (periodic) {
...
gps2red <- function (bspl, y, w = NULL) {
  y <- as.double(y)
  X <- bspl$B
  n <- X@Dim[1L]
  ...
  list(n = n, Z = Z, z = z, L = L, f = f, minRSS = minRSS, E = E)
}

gps2DR <- function (bspl, red, tol = 1e-6) {
  t0 <- Sys.time()
  q <- ncol(red$E)
  dbar <- .Call("C_MeanDR", red$E, PACKAGE = "gps")
  dq <- .Call("C_MinDR", red$E, tol, PACKAGE = "gps")
  d1 <- .Call("C_MaxDR", red$L, bspl$Dt, tol, PACKAGE = "gps")
...
GridPWLS <- function (bspl, red, rho) {
  .Call("C_GridPWLS", red$Z, red$L, bspl$S, red$z, t.default(bspl$C), rho,
        PACKAGE = "gps")
}
...
gps2GS <- function (x, y, w = NULL, xt, d = 4, m = 2, gps = TRUE,
                    periodic = FALSE, ng = 20) {
  bspl <- gps2bspl(x, xt, d, m, gps, periodic)
  red <- gps2red(bspl, y, w)
  DR <- gps2DR(bspl, red)
  ...
  eqn <- list(B = bspl$B, S = S, BWB = BWB, BWy = red$z, C = bspl$C)
  list(eqn = eqn, eigen = DR, rho.lim = rho.lim, E = red$E, pwls = pwls)
}

DemoRhoLim <- function (gps2fit, plot = TRUE) {
  DR <- ExactDR(gps2fit$E)
  approx.DR <- gps2fit$eigen[c("approx.values", "secs")]
  names(approx.DR) <- c("values", "secs")
  q <- length(DR$values)
  rho.min.star <- gps2fit$rho.lim$min
  rho.max.star <- gps2fit$rho.lim$max
  redf.min <- 0.01 * q
  redf.max <- 0.99 * q
  ...
  rho.max.heuristic <- gps2fit$rho.lim$max.heuristic
